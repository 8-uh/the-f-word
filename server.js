...
router.use(function(req, res, next) {
  res.header('Access-Control-Allow-Origin', 'http://thefword.codevinsky.com');
...
router.get('/', function(req, res) {
  ...
  Fuck.find({
    ...
    created: -1
  }).limit(1).exec(function(err, mostRecent) {
    if (!err) {
...
router.get('/singleFucks', function(req, res) {
  ...
  SingleFuck.find({
    ...
    if (!err) {
      mostRecent[0].ngrams.push({
        term: 'Accumulated Fucks',
        tf: mostRecent[0].total
      });
      res.json(mostRecent[0]);
    } else {
      console.log(err);
...
app.use(function(req, res, next) {
  ...
  if (!/codevinsky.com/.test(req.headers.host)) {
    console.log('not coming from codevinsky.com... redirecting');
    res.redirect(301, 'http://thefword.codevinsky.com');
  } else {
    next();
  }
});
